...
export class PlantComponent implements OnInit {
  ...
  //dataSource = [];
  dataSource: MatTableDataSource<PlantData>;
  ...
  HideColumnsAs: string[] = ['companyid', 'countryid', '_id', '__v'];
  getDisplayedColumns() {
  ...
  plant_code: FormControl;
  plant_name: FormControl;

  createFormControlsplant() {
    ...
    this.plant_code = new FormControl('', Validators.required);
    this.plant_name = new FormControl('', Validators.required);
  }
        ...
        if (apipath['server'] !== undefined) {
          this.httpClient.get(apipath['server'] + '/api/manual/company').subscribe(
            (data: any[]) => {
              ...
              //console.log(Object.keys(this.PlantsData[0]).length);
              if (Object.keys(this.PlantsData).length > 0) {
                // console.log(this.PlantsData);
                // console.log(Object.keys(this.PlantsData[0]).length);
                for (let i = 0; i < Object.keys(this.PlantsData[0]).length; i++) {
                  //if (typeof this.PlantsData[i][Object.keys(this.PlantsData[0])[i]] === 'string') {
                  //console.log(Object.keys(this.PlantsData[0])[i]);
                  this.vdisplayedColumns.push(Object.keys(this.PlantsData[0])[i]);
                }
                //}
                //console.log(this.vdisplayedColumns);
                this.vdisplayedColumns.push('star');
                this.gotData = true;
                this.dataSource = new MatTableDataSource(this.PlantsData);
    ...
    this.httpClient.get('configs/apix/api_server.json').subscribe(
      apipath => {
        if (apipath['server'] !== undefined) {
          ...
          this.httpClient.post(apipath['server'] + '/api/manual/plant', this.plantform.value).subscribe(
            (data: any[]) => {
  ...
  updateRow(element) {
    ...
    this.GetLocationList(element.stateid);
    ...
    this.plantform.patchValue({
      ...
      plant_code: element.plantcode,
      plant_name: element.plantname
    });
  }
